...
| Deterministic arbitration / adversarial resolution | IMPLEMENTED / A4.1 ACCEPTANCE CANDIDATE | [A4.1](TIAF_A4_1_DETERMINISTIC_CHALLENGE_ARBITRATION.md) implements bounded primary/counter theses, eight challenge families, explicit deterministic arbitration, preserved dissent/A2 `NO_TRADE`, seven non-action dispositions and offline replay; no model/acquisition bridge or trade-expression authority |
...
| Bounded open-world A4 research loop | PARTIAL / PLANNED | A4.1 emits non-executing typed evidence-need placeholders; the future Planner bridge, ordinary source/PIT/authority/entitlement admission and successor projection remain unimplemented; model prior is not factual evidence |
