...
def main():
    ...
    docx = DOCS / "TI_FM_LFDE_Market_State_Forecasting_Thesis.docx"
    ...
    with zipfile.ZipFile(docx) as archive:
        ...
        assert figures == [f"{i:02}" for i in range(1, 33)]
        ...
        assert len(root.findall(".//w:drawing", NS)) == 3
    ...
    for finding in range(1, 25):
        assert f"FF-{finding:02}." in text
    ...
    classes = collections.Counter(re.findall(r"^## FF-\d+\. .+ — ([A-Z_]+)$", source, re.M))
    assert dict(classes) == {
    ...
    # Verify teaching arithmetic, not model performance or statistical acceptance.
    assert abs(0.240 - 0.226 - 0.014) < 1e-12
    assert abs(0.6 * 0.3**2 + 0.4 * 0.7**2 - 0.25) < 1e-12
    assert abs(0.6 * 0.4**2 + 0.4 * 0.6**2 - 0.24) < 1e-12
    cases = re.split(r"^## Case \d+ —", source, flags=re.M)[1:]
    assert len(cases) == 10
    for case in cases:
        case = case.split("# 33.")[0]
        for field in (
            "State",
            "Explicit factors",
            "Latent contribution",
    ...
    for filename in ("incremental", "rotation", "cost"):
        ...
        json.dumps(
            {
                ...
                "raster_charts_with_editable_svg": 3,
                "worked_cases": 10,
                "faq_answers": 29,
