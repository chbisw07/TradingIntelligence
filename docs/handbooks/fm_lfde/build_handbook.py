...
class Handbook:
    ...
    def build(self):
        ...
        self.para("TIAF  /  FM–LFDE  /  ILLUSTRATED REFERENCE", "Eyebrow")
        self.para("Market-State Forecasting", "Title")
        self.para(
            "Explicit evidence. Learned representations.\nGoverned change.".replace("\n", "\\n"),
            "Subtitle",
        )
        self.para(
            "FM-LFDE: A Multimodal Market-State Forecasting Framework with Latent "
            "Factor Discovery and Governed Self-Correction",
            "Heading2",
        )
        self.para("NON-NORMATIVE THESIS • 14 SEPTEMBER 2026 • ASIA/KOLKATA", "Eyebrow")
        self.para(
            "A6 FROZEN • A7 ACCEPTANCE PAUSED\\nFM/LFDE ARCHITECTURE DRAFT "
        ...
        self.para(
            "35 chapters • 32 figures • 10 worked scenarios • 29 substantial FAQ answers "
            "• 24 reconciliation findings. Numerical model results, symbols, cohorts, "
        ...
        self.para(
            "FM means Forecasting Module, not foundation model. Inputs are structured "
            "PIT data, never chart screenshots. Deterministic controls remain visible; "
            "TM retains action authority and the broker retains execution truth."
        )
        ...
        self.para(
            "Chapters 1–6: foundations and tensors. 7–14: explicit/latent representations "
            "and modalities. 15–23: state, outcomes, evaluation and identifiability. "
            "24–31: governance, replay, failure, delivery and prior art. "
            "32: ten complete scenarios. 33: FAQ. 34: glossary and sources. "
            "35: findings, not applied changes. Diagrams are editable Word tables/flow "
            "boxes; three illustrative plots have editable SVG and data sources.",
            "Source",
        )
    ...
    def write(self):
        ...
        header = el("w:hdr")
        self.para(
            "TI  /  FM–LFDE MARKET-STATE FORECASTING                         DRAFT REFERENCE",
            "Header",
            header,
        )
        footer = el("w:ftr")
        ...
        for tag, text in [
            (
                "title",
                "FM-LFDE Market-State Forecasting Thesis — Illustrated Reference",
            ),
            ("creator", "TradingIntelligence"),
            (
                "description",
                "Non-normative FM-LFDE thesis. Architecture draft; runtime not implemented. "
                "Synthetic illustrations only.",
            ),
        ]:
            ET.SubElement(core, "{http://purl.org/dc/elements/1.1/}" + tag).text = text
        output = DOCS / "TI_FM_LFDE_Market_State_Forecasting_Thesis.docx"
