...
class Handbook:
    ...
    def build(self):
        ...
        self.para("TIAF  /  A7  /  COMPLETE CONCEPTUAL REFERENCE", "Eyebrow")
        self.para("Forecasting, Evaluation\nand Learning".replace("\n", "\\n"), "Title")
        self.para("Reasoning about tomorrow without rewriting yesterday", "Subtitle")
        self.para("User-facing thesis and visual reference handbook", "Heading2")
        self.para("NON-NORMATIVE COMPANION • 14 SEPTEMBER 2026 • ASIA/KOLKATA", "Eyebrow")
        self.para(
            "A6 frozen • A7 architecture draft • A7 thesis created"
            "\\nA7 runtime NOT_IMPLEMENTED • Thesis / architecture reconciliation next",
            "Callout",
        )
        self.para(
            "A complete conceptual reference to forecasting, evaluation and governed learning. "
            "The architecture remains the controlling draft. This thesis neither accepts it "
            "nor implements or trains a model. All model names, cohorts, market observations, "
            "performance figures and scenario approvals in teaching examples are invented."
        )
        self.para(
            "FORECASTING: What may happen?\\nEVALUATION: Was the estimate useful?"
            "\\nLEARNING: Which improvements earn future use?",
            "Callout",
        )
        self.para(
            "36 chapters • 29 FAQ answers • 10 worked scenarios • 17 reconciliation findings. "
            "Editable diagrams and tables accompany detailed narrative. Statistical charts "
            "are explicitly illustrative, not live-data results or a forecast guarantee."
        )
        self.para(
            "A2–A6 remain visible controls. A7 is shadow/advisory only in proposed v1. "
            "TM retains risk, capital, quantity and action authority; the broker executes."
        )
        ...
        self.para(
            "Chapters 1–7: meaning and measurement. 8–18: data and evaluation. "
            "19–23: governed model lifecycle. 24–32: consumers, replay and safety. "
            "33: ten worked cases. 34: FAQ. 35: evolution. 36: findings. "
            "Appendix A: source authority, glossary and study guide. All figures are "
            "editable Word tables except the two plotted charts, whose editable SVG/data "
            "sources accompany the document."
        )
    ...
    def write(self):
        ...
        header = el("w:hdr")
        self.para(
            "TI  /  FORECASTING, EVALUATION & LEARNING                      DRAFT USER REFERENCE",
            "Header",
            header,
        )
        footer = el("w:ftr")
        ...
        for tag, text in [
            (
                "title",
                "TI Forecasting, Evaluation & Learning Thesis — Complete Conceptual Reference",
            ),
            ("creator", "TradingIntelligence"),
            (
                "description",
                "Non-normative A7 thesis. Architecture draft; runtime not implemented. "
                "Synthetic illustrations only.",
            ),
        ]:
            ET.SubElement(core, "{http://purl.org/dc/elements/1.1/}" + tag).text = text
        output = DOCS / "TI_Forecasting_Evaluation_Learning_Thesis.docx"
