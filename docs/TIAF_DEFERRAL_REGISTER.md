...
| DEF-012 | News, filings, fundamentals, macro, sector, and peer evidence adapters | A1 | DEPENDENCY_DEFERRAL | Production acquisition needs explicit source rights, identity, time, and quality semantics. | Source selection, licensing, identity, time and quality contracts | HIGH | A3 CLOSURE | DEFERRED | A3.4–A3.6 provide canonical contracts/gateways. A3.6.1 defines provider declarations, routing, normalization, and progressive enrichment and selects Tapetide conditionally as the first candidate; production access/licensing, adapters, and live acceptance remain unimplemented and acquisition stays outside Agents. |
