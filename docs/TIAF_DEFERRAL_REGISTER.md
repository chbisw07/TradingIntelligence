...
A6 is ARCHITECTURE RECONCILED / THESIS RECONCILED / ACCEPTANCE NEXT, not runtime
implementation or deferral closure. The
...
| DEF-006 | Option-expression selection and strategy templates | A0; reaffirmed A2.7/A2.8 | CAPABILITY_DEFERRAL | Factual underlying/chain evidence must precede contract and strategy choice. | Independent A6 architecture acceptance, admitted thesis, qualified option/timing evidence and versioned liquidity policy | HIGH | A6 / SEPARATE STRATEGY AND A7 FOLLOW-UP | PLANNED | Architecture/thesis reconciled, acceptance next, runtime NOT_IMPLEMENTED. Initial delivery is advisory long single-leg CE/PE with explicit absence; broader strategies remain beyond v1. TF-05 rejected-candidate evidence short-circuit is deferred to separate post-baseline policy review with equivalence/replay proof; full-window requirements retained. A7 comparison is later; no TM bypass. SigmaDSL is not a prerequisite. |
| DEF-007 | Market-calendar-aware source-observation recency | A1; reviewed A5 architecture and A6 thesis reconciliation | DEPENDENCY_DEFERRAL | Wall-time age alone cannot determine closed-market observation acceptability. | Exchange calendar/session contract and separately qualified precise expiry timing | HIGH | A6.1 TIMING CONTRACT / BEFORE SESSION-SENSITIVE MONITORING | DEFERRED | A5 age/intent semantics unchanged. A6.1 requires explicit sourced DAY session and expiration qualification before A6.2; expiry date cannot imply an instant. Last trading cutoff is separately attributed context, not inferred from expiration or a default v1 gate. No calendar/session engine, source procurement or live feasibility is claimed. |
