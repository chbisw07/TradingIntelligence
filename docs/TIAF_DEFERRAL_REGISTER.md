...
The [A6 architecture reconciliation](TIAF_A6_RECONCILIATION_RECORD.md) narrows
initial DEF-006 delivery to advisory long single-leg CE/PE and removes any
SigmaDSL prerequisite from TI A6 v1. DEF-025 remains deferred for separate
strategy/product review. Older audit tables below mentioning “A6 closure” for
SigmaDSL retain historical context; this current canonical resolution governs.
A6.1–A6.4 are ACCEPTED / DONE, A6 is FROZEN at `tiaf-a6-baseline`, and
`expression.assess` is PUBLISHED as captured-read only. The
[A6.4 closure](TIAF_A6_4_FINAL_HARDENING_ACCEPTANCE_CORPUS_FREEZE_READINESS.md)
provided the baseline freeze evidence and is not closure of
the broader A6 deferrals. The
[independent acceptance](TIAF_A6_TRADE_EXPRESSION_INTELLIGENCE_ARCHITECTURE_ACCEPTANCE.md)
preserves every explicit limitation. The
[thesis reconciliation](TIAF_A6_THESIS_ARCHITECTURE_RECONCILIATION.md) retains
strict source-time requirements, clarifies exact expiry/session qualification
and defers candidate short-circuit TF-05 under DEF-006's later policy work.
DEF-054 rich reporting/NLP/Web, DEF-010 monitoring runtime, DEF-039–046 models,
DEF-044 A7 forecasts and DEF-056 multi-leg A5 remain open. Historical audit
labels are not mandatory initial-A6 integration gates. IDs/statuses unchanged.
...
| DEF-006 | Option-expression selection and strategy templates | A0; reaffirmed A2.7/A2.8 | CAPABILITY_DEFERRAL | Factual underlying/chain evidence must precede contract and strategy choice. | Accepted A6 architecture, admitted thesis, qualified option/timing evidence and versioned liquidity policy | HIGH | POST-A6 STRATEGY AND A7 FOLLOW-UP | PLANNED | A6.1–A6.4 and governed captured-read `expression.assess` are frozen at `tiaf-a6-baseline`. Delivered v1 is advisory long single-leg CE/PE with explicit absence. Multi-leg construction, option selling, futures/cross-asset expressions and broader strategy templates remain open. TF-05 rejected-candidate evidence short-circuit is deferred to separate post-baseline policy review with equivalence/replay proof; full-window requirements retained. A7 comparison is later; no TM bypass. SigmaDSL is not a prerequisite. |
| DEF-007 | Market-calendar-aware source-observation recency | A1; reviewed A5 architecture and A6 thesis reconciliation | DEPENDENCY_DEFERRAL | Wall-time age alone cannot determine closed-market observation acceptability. | Exchange calendar/session contract and separately qualified precise expiry timing | HIGH | BEFORE SESSION-SENSITIVE MONITORING / LIVE A6 | DEFERRED | A5 age/intent semantics unchanged. A6.1 now provides explicit sourced DAY-session and expiration-qualification contracts; expiry date cannot imply an instant. The calendar/session engine and live qualified source remain absent. Last trading cutoff is separately attributed context, not inferred from expiration or a default v1 gate. No source procurement or live feasibility is claimed. |
...
| DEF-014 | Authoritative option-chain market-event timestamp | A1/A2.7; A6 thesis reconciliation | DEPENDENCY_DEFERRAL | Dhan chain acquisition time cannot alone satisfy strict A6 market-age gates. | Provider field or independently accepted timestamp source, bound to contract/field with availability and qualification provenance | MEDIUM | BEFORE LIVE QUALIFIED SELECTION | DEFERRED | A1/A2.7 retrieval-time semantics preserved. A6.1 now defines/tests additive qualification; absent live source yields insufficient, not WAIT or invented freshness. No acquisition-qualified fallback or timestamp source is approved here. |
...
| DEF-025 | Indicator strategy transitions and SigmaDSL rule integration | A2.4; clarified A6 architecture reconciliation | CAPABILITY_DEFERRAL | Factual indicator state is separate from entry/exit strategy interpretation; TI A6 is not a programmable strategy runtime. | Separately accepted strategy/rule contracts and product/evaluation boundary, not an A6 dependency | HIGH | SEPARATE STRATEGY / SIGMATRADER REVIEW | DEFERRED | SigmaDSL is explicitly outside TI A6 v1: no parser, compiler, runtime or compatibility-layer prerequisite. Future programmable SigmaTrader is a separate product direction. Native A6 typed policies need no DSL. Strategy transitions must still consume, not redefine, indicator facts; historical A6-closure revisit wording is superseded by this clarification, not implementation. |
