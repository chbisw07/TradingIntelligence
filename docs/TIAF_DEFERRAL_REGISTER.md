...
status enums. Source foundation is immediate work, not a new deferral. No status
or stable ID changes in this pass; historical closure tables remain snapshots.
...
| DEF-003 | Governed TIAF public capability/service/API surface | A0; clarified A3 closure audit and post-A3 pass 1 | CAPABILITY_DEFERRAL | Foundation milestones expose packages, an in-process A3.9 seam and diagnostics, not a consolidated governed consumer boundary. | Stable intelligence contracts, curated capability/version/effect descriptors, trusted caller admission, authorization/data rights, and later transport authentication/operations | MEDIUM | POST-A3 LOCAL / A8 | PLANNED | [Consolidation](TIAF_POST_A3_CONSOLIDATION_AND_REPLANNING.md) selects foundation -> narrow local facade/lifecycle -> deterministic A4. Local facade is mandatory before Shell/public consumers, not a dependency of internal source projection. Remote service/API and TM integration remain A8 as required, with A10 operations. Both tracks retain this ID; no facade runtime is implemented. |
