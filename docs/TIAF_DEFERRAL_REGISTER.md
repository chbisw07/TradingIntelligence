...
| DEF-003 | Governed TIAF public capability/service/API surface | A0; clarified A3 closure audit and post-A3 pass 1 | CAPABILITY_DEFERRAL | Foundation milestones expose packages, an in-process A3.9 seam and diagnostics, not a consolidated governed consumer boundary. | Stable intelligence contracts, curated capability/version/effect descriptors, trusted caller admission, authorization/data rights, and later transport authentication/operations | MEDIUM | POST-A3 / A8 | PLANNED | Two delivery tracks under this stable ID: local facade/catalog architecture is settled by [post-A3 pass 1](TIAF_POST_A3_DEEP_ARCHITECTURE_PASS_1_CORE_CAPABILITY_BOUNDARY.md); narrow implementation is recommended before A4 and required before Shell/public consumer use, but does not block design pass 2. Remote service/API and TM integration remain A8 as required, with A10 operations. No facade/catalog runtime is implemented or closed by this review; diagnostics are not the API. |
...
| DEF-047 | Automatic benchmark and sector mapping | A2.8 | DEPENDENCY_DEFERRAL | No accepted provider-neutral sector/classification source exists. | Normalized classification evidence, versioning and mapping policy | HIGH | A3 CLOSURE | DEFERRED | A2.8 and A3.6 require explicit caller/provider-supplied benchmark/sector identity. A3.6 adds versioned effective-time mapping contracts but no automatic mapper. |
...
| DEF-054 | User-facing explanation, source-citation and report rendering fabric | A3.9; reaffirmed A3 closure; clarified post-A3 pass 2 | CAPABILITY_DEFERRAL | Internal claim/evidence/source lineage exists, but citation numbering, hyperlinks, bibliography, compression, source labels and CLI/Web report projection remain separate presentation work. | Approved Core source semantics plus their bounded projection, governed source references, public capability facade and redaction/presentation policy | HIGH | POST-A3 UX | DEFERRED | [Pass 2](TIAF_POST_A3_DEEP_ARCHITECTURE_PASS_2_SOURCE_AUTHORITY_PROVENANCE_CONTRADICTION.md) separates approved Core source/comparability/independence semantics from this UX deferral. The semantic foundation is an immediate pre-A4 runtime prerequisite, not completion of rendering or a new deferred A3 capability. No renderer is implemented; preserve contradictions, scoped authority and original lineage without re-research or making UI prose canonical truth. |
