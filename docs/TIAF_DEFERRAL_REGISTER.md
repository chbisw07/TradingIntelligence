...
| DEF-054 | User-facing explanation, source-citation and report rendering fabric | A3.9; reaffirmed A3 closure; clarified post-A3 pass 2 and A4 closure | CAPABILITY_DEFERRAL | Internal claim/evidence/source/A4 lineage exists, but citation numbering, hyperlinks, bibliography, compression, source labels and CLI/Web report projection remain separate presentation work. | Accepted Core source semantics, A4 lineage, governed local facade, and a bounded redaction/presentation policy | HIGH | POST_A4_PRE_A5 SHELL / LATER WEB | PLANNED | Split within this stable ID. A4 closure plans only minimal captured-source and A4 explanation rendering with command-first Shell architecture/v0.1. Rich bibliography/report/Web work remains deferred until a consumer requires it. Preserve contradictions, scoped authority, access controls and original lineage without re-research; no renderer is implemented by A4 closure. |
