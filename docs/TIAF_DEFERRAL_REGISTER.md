...
A6.1–A6.2 are ACCEPTED / DONE, A6.3 is ACTIVE / NEXT, and `expression.assess`
remains NOT_PUBLISHED. This is not closure of the broader A6 deferrals. The
...
| DEF-006 | Option-expression selection and strategy templates | A0; reaffirmed A2.7/A2.8 | CAPABILITY_DEFERRAL | Factual underlying/chain evidence must precede contract and strategy choice. | Accepted A6 architecture, admitted thesis, qualified option/timing evidence and versioned liquidity policy | HIGH | A6 / SEPARATE STRATEGY AND A7 FOLLOW-UP | PLANNED | Architecture/thesis and A6.1–A6.2 internal foundation/evaluator accepted; A6.3 publication active/next; capability remains unpublished. Initial delivery is advisory long single-leg CE/PE with explicit absence; broader strategies remain beyond v1. TF-05 rejected-candidate evidence short-circuit is deferred to separate post-baseline policy review with equivalence/replay proof; full-window requirements retained. A7 comparison is later; no TM bypass. SigmaDSL is not a prerequisite. |
